...
model.add(Conv2D(filters=64, kernel_size=(1, 6),
                activation='relu', input_shape=(60,6,1)))
...
model.add(Dense(units=100, kernel_regularizer=L1L2(0.00001, 0.00002),
kernel_initializer=relu_initializer))
model.add(BatchNormalization())
model.add(Dropout(0.2))
model.add(Dense(units=200, kernel_regularizer=L1L2(0.00001, 0.00002),
kernel_initializer=relu_initializer))
model.add(BatchNormalization())
model.add(Dropout(0.2))
...
history = model.fit(feature_set, labels, epochs = 300, batch_size = 64, \
